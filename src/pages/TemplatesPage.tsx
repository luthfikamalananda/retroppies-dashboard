...
export default function TemplatesPage() {
    ...
    useEffect(() => {
        if (dialogOpen) {
            if (editTarget) {
                setValue('tenantId', editTarget.tenantId);
                setValue('isDefault', editTarget.isDefault ?? false);
            } else {
                if (activeTenantId) setValue('tenantId', activeTenantId);
    ...
    const uploadMutation = useMutation({
        mutationFn: ({ tenantId, displayFile, productionFile, isDefault }: { tenantId: number; displayFile?: File; productionFile?: File; isDefault: boolean }) =>
            editTarget
                ? templatesApi.update(editTarget.id, tenantId, Number(layoutId), displayFile ?? null, productionFile ?? null, isDefault, (pct) => setUploadProgress(pct))
                : templatesApi.upload(tenantId, Number(layoutId), displayFile ?? null, productionFile ?? null, isDefault, (pct) => setUploadProgress(pct)),
        onSuccess: () => {
    ...
    return (
        <Box>
            <input ref={fileInputRef} type="file" accept="image/png" hidden onChange={handleFileSelect} />
            <input ref={productionFileInputRef} type="file" accept="image/png" hidden onChange={handleProductionFileSelect} />

            <Breadcrumbs sx={{ mb: 2 }} aria-label="breadcrumb">
                <Link component={NavLink} to="/app/dashboard" sx={{ display: 'flex', alignItems: 'center', color: colors.base['grey'], textDecoration: 'none' }}>
                    <HomeIcon sx={{ fontSize: 18 }} />
                </Link>
                <Link component={NavLink} to="/app/layouts" sx={{ color: colors.base['grey'], fontSize: 14, textDecoration: 'none' }}>
                    Layout
                </Link>
                        ...
                        <Grid size={{ xs: 12, sm: 6 }} key={template.id}>
                            <Card
                                sx={{
                                    display: 'flex',
                                    borderRadius: 2,
                                    border: template.isDefault ? `2px solid ${colors.brand[500]}` : `1px solid ${colors.border['light']}`,
                                    boxShadow: 'none',
                                    overflow: 'hidden',
            ...
            <Dialog open={dialogOpen} onClose={closeDialog} maxWidth="sm" fullWidth>
                ...
                <DialogContent dividers sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    ...
                    <TextField
                        ...
                        size="small"
                        fullWidth
                        value={watch('isDefault') ? 'true' : 'false'}
                        onChange={(e) => setValue('isDefault', e.target.value === 'true', { shouldValidate: true })}
                    >
                            ...
                            {fileError ? (
                                ...
                                <Stack sx={{ gap: 1, alignItems: 'center' }}>
                                    <Box
                                        ...
                                        alt="preview display"
                                        sx={{ maxWidth: '100%', maxHeight: 220, objectFit: 'contain', border: `1px solid ${colors.border['light']}`, borderRadius: 2, bgcolor: colors.base['section'] }}
                                    />
                                    <Box sx={{ width: '100%' }}>
                                        {selectedFile ? (
                                            <>
                                                <Typography sx={{ fontWeight: 600, fontSize: 13, color: colors.base['black'] }}>{selectedFile.name}</Typography>
                                                <Typography sx={{ fontSize: 12, color: colors.base['grey'] }}>{(selectedFile.size / 1024 / 1024).toFixed(2)} MB</Typography>
                                            </>
                                        ) : (
                                            <Typography sx={{ fontSize: 12, color: colors.base['grey'] }}>File existing (dari server)</Typography>
                                        )}
                                    </Box>
                                    <Button size="small" onClick={() => { setSelectedFile(null); setPreviewUrl(null); setExistingDisplayUrl(null); setFileError(''); fileInputRef.current?.click(); }} disabled={uploadMutation.isPending} sx={{ textTransform: 'none', alignSelf: 'flex-start' }}>Ganti File</Button>
                                </Stack>
                            )}
                        </Box>
                                ...
                                <Stack sx={{ gap: 1, alignItems: 'center' }}>
                                    ...
                                    <Box sx={{ width: '100%' }}>
                                        {selectedProductionFile ? (
                                            <>
                                                <Typography sx={{ fontWeight: 600, fontSize: 13, color: colors.base['black'] }}>{selectedProductionFile.name}</Typography>
                                                <Typography sx={{ fontSize: 12, color: colors.base['grey'] }}>{(selectedProductionFile.size / 1024 / 1024).toFixed(2)} MB</Typography>
                                            </>
                                        ) : (
                                            <Typography sx={{ fontSize: 12, color: colors.base['grey'] }}>File existing (dari server)</Typography>
                                        )}
                                    </Box>
                                    <Button size="small" onClick={() => { setSelectedProductionFile(null); setProductionPreviewUrl(null); setExistingProductionUrl(null); setProductionFileError(''); productionFileInputRef.current?.click(); }} disabled={uploadMutation.isPending} sx={{ textTransform: 'none', alignSelf: 'flex-start' }}>Ganti File</Button>
                                </Stack>
                            )}
                        </Box>
                    </Stack>
                    ...
                    {uploadProgress !== null && (
                        <Box sx={{ mt: 1 }}>
                            ...
                            <Typography variant="caption" sx={{ color: colors.base['grey'] }}>{uploadProgress}%</Typography>
                        </Box>
                    )}
                </DialogContent>
                <DialogActions sx={{ px: 3, py: 2, gap: 1 }}>
                    ...
                    <Button
                        ...
                        onClick={handleSubmit((values) => {
                            const displayFile = selectedFile || undefined;
                            const productionFile = selectedProductionFile || undefined;
                            ...
                            uploadMutation.mutate({ tenantId: values.tenantId, displayFile, productionFile, isDefault: values.isDefault });
                        })}
                        sx={{
                            bgcolor: colors.brand[500],
                            '&:hover': { bgcolor: colors.brand[600] },
